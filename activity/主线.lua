...
local 前往活动,购买mana=黑板:createScene(),黑板:createScene()
...
前往活动:getStartTrigger():setRule(
	function(blackboard)
		return blackboard:getValue("当前流程")=="前往活动" and blackboard:getValue("当前场景")=="主页面"
	end
	)
...
寻找章节:getStartTrigger():setRule(
	function(blackboard)
		return blackboard:getValue("当前流程")=="寻找章节" and blackboard:getValue("当前场景")=="活动关卡页面"
	end
	)
...
领取活动任务奖励:getStartTrigger():setRule(
	function(blackboard)
		return blackboard:getValue("当前流程")=="领取活动任务奖励" and blackboard:getValue("当前场景")=="活动关卡页面"
	end
	)
-->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
local function 全家桶(blackboard)
	...
	if Data.讨伐证交换:getandCmpColor() then print('讨伐证交换')
		Data.主页_我的主页:Click(1)
		blackboard:setValue('初始化页面',true)
		return blackboard:setValue("当前流程","前往活动")		
	end
...
local 初始化表_主页={'冒险','主线关卡','行会','任务页面','礼物','公会之家','技能强化','地下城','竞技场','商店','简介','活动任务页面','活动页面','活动关卡页面'}
	...
	_K:Switchkeep()
		...
		if aim=='主页面' then print('已在主页')
			...
			if blackboard:getValue('任务开始')==false then
				blackboard:setValue("当前流程","购买mana")
...
前往活动:getDoingBehavior():setServer(
	function(blackboard) print('前往活动')
	_K:Switchkeep()
		local aim=scene.初始化页面()
		if aim=='主页面' then
			Data.主页_冒险:Click()
		...
		elseif aim=='活动页面' then 
			Data.活动页面_活动关卡:Click()
		elseif aim=='活动关卡页面' then
			blackboard:setValue('初始化页面',false)
			blackboard:setValue('当前场景','活动关卡页面')
...
寻找章节:getDoingBehavior():setServer(
	...
	_K:Switchkeep()
		...
		local 识别结果=scene.识别活动地图()
		Print(识别结果)
		if 识别结果 ~= nil then
			Data.活动一图[识别结果]:Click(1)
...
寻找关卡:getDoingBehavior():setServer(
	...
	_K:Switchkeep()
		if blackboard:getValue('当前关卡') >= blackboard:getValue('目标关卡') then
			Data.关卡页面_取消:Click();识别关卡次数=0;
			blackboard:setValue('当前关卡',1)
			blackboard:setValue('当前流程','领取活动任务奖励')
			return blackboard:setValue("当前场景","活动关卡页面")
		end
		if Data.关卡页面:getandCmpColor() then
			...
			if Data.地图三星:getandCmpColor() then print('地图没三星')
				...
				elseif aim=='卡关' then	print('等级不够卡关了')
					blackboard:setValue('初始化页面',true);识别关卡次数=0
					blackboard:setValue('当前流程','领取活动任务奖励')
					return blackboard:setValue("当前场景","活动关卡页面")
				end
			end
...
领取活动任务奖励:getDoingBehavior():setServer(
	function(blackboard) print('领取活动任务奖励');Data.瞎点:Click(1)
	_K:Switchkeep()
		if Data.活动关卡页面:getandCmpColor() then
			Data.活动关卡_任务:Click(2)
		end
		if Data.活动任务页面:getandCmpColor() then
			behavior.领取任务奖励()
		...
		if Data.主菜单:getandCmpColor() then
			...
			_K:Switchkeep()
				if not Data.简介:getandCmpColor() then
					...
					for i=1,10 do Data.简介:Click(0.1) end;behavior.写入ID(blackboard);behavior.保存数据()
...
退出账号:getDoingBehavior():setServer(
	...
	_K:Switchkeep()
		if Data.主页面:getandCmpColor() or Data.活动关卡页面:getandCmpColor()then
...
登录账号:getDoingBehavior():setServer(
	function(blackboard) print('登录账号')  已退出=false
		local 读取账号=behavior.验证账号('activity')
